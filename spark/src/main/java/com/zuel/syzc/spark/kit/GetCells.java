...
public class GetCells {
     ...
     * @return 两点之间距离
     */
    private double getDistanceBetweenPoints(double lon1, double lat1, double lon2, double lat2) {
     ...
     * @return Tuple2<基站CellId, 位置标记> 0:不在范围内, 1:在范围内
     */
    public List<Tuple2<String, Integer>> getCellsInPolygon(List<Point> points) {
        Iterator<Row> data = getData();
        List<Tuple2<String, Integer>> baseStationCells = new ArrayList<>();
        // 构造多边形的外包矩形
        ...
        while (data.hasNext()) {
            ...
            double baseStationLatitude = Double.parseDouble(row.getString(1));
            String cellId = row.getString(2).split("-")[1];
            Point baseStation = new Point(baseStationLongitude, baseStationLatitude);
     ...
     * @return 是否在多边形内
     */
    private boolean isPointInPolygon(Point point, List<Point> points) {
